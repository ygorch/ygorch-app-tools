           ...
           <div className="relative z-10 p-8 flex flex-col md:flex-row items-start md:items-center gap-6">
              <div className="w-24 h-24 rounded-2xl bg-black/30 backdrop-blur-md flex items-center justify-center border border-white/10 overflow-hidden">
                 {list.thumbnailBlob ? (
                    ...
                    React.createElement(LucideIcons[list.iconName] || LucideIcons.Gift, { size: 40 })
                 )}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                 <button onClick={exportToCSV} className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-xs font-medium border border-white/10">CSV</button>
                 <button onClick={exportToJSON} className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-xs font-medium border border-white/10">JSON</button>
                 <button onClick={exportToXLSX} className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-xs font-medium border border-white/10">Excel</button>
                 <button onClick={shareToWhatsapp} className="px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg text-xs font-medium flex items-center gap-2">
                    <LucideIcons.Share2 size={14} /> Share
                 </button>
              </div>
           </div>
        </div>
                    ...
                    return (
                        <GlassCard key={item.id} className="flex items-center gap-4 group">
                             ...
                             <div className="w-12 h-12 rounded-xl bg-white/5 flex-shrink-0 flex items-center justify-center overflow-hidden border border-white/5">
                                 {item.url ? (
                                    <img src={getFaviconUrl(item.url) || ''} alt="icon" className="w-8 h-8 object-contain opacity-80" />
                                 ) : (
